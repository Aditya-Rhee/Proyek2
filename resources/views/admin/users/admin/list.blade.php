                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h4>List Data Pengguna Admin</h4>
                                        <div class="card-header-action" style="margin-right: 10px">
                                            <a href="{{ route('admin.pengguna.create') }}" class="btn btn-primary">Tambah User</a>
                                        </div>
                                                    ...
                                                    @foreach ($users as $index => $user)
                                                        <tr>
                                                            ...
                                                            <td class="text-center" style="display: flex; align-items: center; gap:10px;">
                                                                <a href="{{ route('admin.pengguna.edit', $user->id) }}" class="btn btn-warning" title="Edit">
                                                                    <i class="fas fa-edit"></i>
                                                                </a>
                                                                <button type="button" class="btn btn-danger text-white btn-delete" data-id="{{ $user->id }}" title="Menghapus Data">
                                                                    <i class="fas fa-trash-alt text-white"></i>
                                                                </button>
                                                                <form id="deleteForm-{{ $user->id }}" action="{{ route('admin.pengguna.delete', ['id' => $user->id]) }}" method="POST" style="display: none;">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                </form>
                                                            </td>
